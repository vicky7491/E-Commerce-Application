                ...
                <div className="relative h-full w-full overflow-hidden bg-[#f0e7db]">
                  {item.type === 'image' ? (
                    ...
                    <div className="relative w-full h-full">
                      <ProtectedVideo src={item.image} className="group" />
        ...
        <AnimatePresence>
          {selectedItem && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 md:p-8 overflow-y-auto" onClick={() => setSelectedItem(null)}>
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                transition={{ type: "spring", damping: 25, stiffness: 300 }}
                className="relative max-w-6xl w-full max-h-[90vh] bg-[#f5f0e9] rounded-2xl overflow-y-auto shadow-xl"
